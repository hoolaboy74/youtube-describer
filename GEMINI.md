...
> [!IMPORTANT]
> 이 프로젝트 루트는 뷰래이터 서비스의 main 브랜치입니다.
...
- **문제:**
    1. 기존 WaveNet 및 Neural2 모델의 음성은 톤이 거의 똑같고 기계적인 조가 강해 장시간 화면 해설 청취 시 피로감이 있었음.
    2. TTS 생성 시 텍스트의 해시값만 캐시 키로 사용하여, 보이스 모델이 변경되더라도 이전 목소리로 생성된 캐시 오디오(Stale Cache)가 지속 서빙되는 캐싱 결함이 존재했음.
    3. 기존 플레이어 해설 속도의 최저 배속이 1.5x로 다소 빠르게 시작되어, 화면 해설에 익숙지 않은 초보 사용자용 1.2x 속도 옵션 요구됨.
- **해결:**
    1. **구글 플래그십 생성형 TTS 모델 도입 (`ko-KR-Chirp3-HD-Sulafat`)**:
        - 14종의 Chirp3-HD 모델 중 딕션이 명확하고 자연스러우며, 월간 100만 자 무료 쿼터를 활용해 비용 효율성이 높은 여성형 모델 `Sulafat`로 업그레이드함.
    2. **보이스 모델 연계형 캐싱 해시 키 도입**:
        - 캐시 키 생성식을 `crypto.createHash('sha256').update(voiceName + ":" + text)`로 재정의하여 보이스 모델 변경 시 캐시가 독립 격리 및 자동 갱신되도록 개선함.
    3. **플레이어 해설 속도 리매핑 및 가독성 최적화**:
        - `PlayerScreenV2.js`에서 배속 단계를 [초보: 1.2, 중수: 1.5, 고수: 2.0, 신: 2.5] 4단계로 전면 리매핑하고, 디폴트 값을 1.2로 수정하여 초보자가 편안히 들을 수 있도록 조율함.
- **상태:** `routes.js`, `test_tts.js`, `PlayerScreenV2.js` 수정 및 eslint 린트/CLI 검증 통과 완료 (2026-08-13)
...
### 102차: 장기 비디오 토큰 오버플로우 방지를 위한 gemini-3.1-pro-preview 모델 및 LOW-res 프레임 모드 전환 배포 (2026-07-13)
- **문제:**
...
### 101차: Q&A(AI 대화) 접근성 개선, 연속 질문 복원, 레이스 컨디션 방어 및 사용자/비디오별 일일 누적 비용 수집(UPSERT) 연동 배포 (2026-07-10)
- **문제:**
    1. **모달 진입 시 스크린리더 낭독 소음**: Q&A 전송 후 답변 수신(또는 에러) 시점에 강제로 `input`에 포커스를 주도록 설계되어 있어, 스크린리더가 답변 TTS와 겹쳐서 다이얼로그 전체 타이틀과 설명을 반복 낭독하는 소음 현상이 지속됨.
    2. **연속 질문 조작의 불편함**: 질문 입력 후 엔터를 쳐서 전송하면 입력창이 `blur()` 처리되어 포커스가 완전히 해제되므로, 연속으로 질문을 작성하려는 시각장애인 유저가 매번 입력창을 다시 탭으로 찾아 들어가야 하는 불편함 상존.
    3. **화면해설-Q&A 간 비동기 레이스 컨디션**: 화면해설(TTS)을 재생하기 위해 비디오를 일시정지시킨 후, 음성을 가져오는 비동기 통신(`axios.post`) 과정 중에 사용자가 Q&A 모달을 열면, 뒤늦게 통신이 완료되어 백그라운드에서 오디오가 재생되고 재생 종료 콜백에 의해 비디오가 멋대로 다시 켜지는 버그 발견.
    4. **Q&A API 누적 비용 및 유저 트래킹 부재**: 사용자가 실시간으로 쏟아내는 Q&A 질문의 API 소모 요금이 수집되지 않았으며, 어드민 비용 통계 테이블에 누가 어떤 영상에서 얼마나 비용을 썼는지 트래킹할 정보가 누락됨.
    5. **비회원 Q&A 요금 남용 방어 부재**: 로그인하지 않은 익명 사용자도 Q&A 기능을 마음대로 사용해 시스템 비용을 과도하게 청구할 수 있는 보안 취약성 존재.
- **해결:**
    1. **Q&A 낭독 소음 해결**: 답변 수신 완료 및 에러 시점에 입력창에 포커스를 주던 `setTimeout` 구문을 완전 제거하여, 모달 타이틀과 입력창 레이블이 중복 낭독되어 TTS 오디오와 뒤섞이는 문제를 해결.
    2. **연속 질문 입력창 포커스 복원**: 질문 전송 즉시 `blur()` 하던 로직을 지우고 `inputRef.current.focus()`를 직후에 호출하여, 텍스트가 비워진 직후 끊김 없이 연속적인 타이핑 및 질문 작성을 지원하도록 흐름 개선.
    3. **TTS-모달 레이스 컨디션 이중 방어**:
        - `playAudioFromUrl` 내부에서 오디오를 실제 재생하기 직전에 `isTtsPlayingRef.current`가 여전히 `true`인지 검사하여 모달 오픈 시의 오디오 재생을 원천 차단.
        - `handleTtsEnd` (오디오 종료/에러 처리기) 내부에서 `isQaModalOpen` 일 경우 즉시 리턴하게 하여 모달 작동 중에 비디오가 뒤늦게 재생(`player.playVideo()`)되는 오작동을 해결.
    4. **사용자별 일일 누적 비용 수집 (`qa_user_daily_costs`) 구축**:
        - 일자(`YYYY-MM-DD`) 기준 `(userId, videoId, logDate)` 복합 유니크 키로 구성된 일일 Q&A 비용 누적 테이블 스키마 신설 및 `upsertQaCost` 로직을 `database.js`에 구축.
        - `videoProcessor.js`에 격리되어 있던 `calculateApiCost` 함수를 `utils.js` 공통 헬퍼로 이관하고 `gemini-3.1-flash-lite` 단가 요율(입력 $0.075 / 1M, 출력 $0.30 / 1M) 반영.
        - API 응답 완료 시 `usageMetadata`에서 토큰을 추출해 한국 시간(KST) 날짜 기준으로 UPSERT 적재하도록 백엔드 처리.
    5. **회원 전용 차단 및 UI 보완**: 백엔드 `/api/video-qa` 라우터에 `requireAuth` 세션 가드를 추가하고, 프론트엔드 버튼의 상태에 따른 비주얼 변화(회색 비활성화 테마 및 `AI와 대화하기 (로그인 필요)` 레이블 전환) 적용 및 모달 진입 전 로그인 검증 알림 주입.
- **상태:** 로컬 수정, DB 자동 마이그레이션 적용, QA 환경 배포 완료 및 `qa_user_daily_costs` 테이블 실물 적재(차주현 회원, 3회 Q&A, 누적 12,598토큰 정상 UPSERT 확인) 성공 (2026-07-10)
...
### 91차: 개인정보 수집 동의 도입, 미인증 회원 5분 생성 제한, 메인화면 후원 제거, 마이페이지 독립 인증 프로세스 구축, 게시판 본인 댓글 수정/삭제 권한 매핑 오류 해결 및 작성자 닉네임 입력 제어 지원 (2026-06-29)
- **문제:**
...
### 90차: QA 전용 독립 검증 환경(QA Server) 구성 및 배포 파이프라인 자동화 (2026-06-24)
- **내용:**
...
### 89차: 자막 미존재 비디오에 대한 Q&A 시 중복 다운로드 네트워크 지연 방지 (Negative Caching 도입) (2026-06-24)
- **문제:**
    1. 자막이 존재하지 않는 영상에 대해 Q&A 질문이 발생할 때마다, 로컬 캐시 디렉토리에 `.vtt` 파일이 없어 캐시 미스로 판정하고 매번 유튜브에 자막 다운로드 요청(`yt-dlp`)을 트리거함.
    2. 불필요한 네트워크 지연이 발생해 사용자 응답 속도가 느려지고, 단시간 내 유튜브에 중복 요청을 보냄으로써 봇 감지 및 IP 차단 위험이 극대화됨.
- **해결:**
    1. **부정 캐싱(Negative Caching) 플래그 도입 및 스킵**: 자막 다운로드 실패 시 `public/subtitles/[videoId].nosub` 빈 파일을 생성하고, Q&A 처리부(`getAdjacentSubtitles`)에서 이 파일이 존재하면 아웃바운드 커넥션을 생성하지 않고 즉시 early return 하도록 구현.
    2. **해설 생성 단계에서의 선제적 캐싱**: 실시간 비디오 처리(`processVideo`) 및 배치 비디오 처리(`processVideoBatch`) 단계에서 자막 획득에 실패했을 때도 `.nosub` 플래그를 사전에 저장하도록 동기화하여 중복 다운로드 시도를 사전에 예방.

### 88차: Q&A 품질 향상 및 대사 인식 기능 보강, 더킹 상태 제어 수정 (2026-06-24)
- **문제:**
    1. Q&A AI 모델에게 영상 정보(T전후 45초의 화면 해설)만 제공되고 실제 비디오 대사(자막) 정보가 주입되지 않아, "주인공이 방금 뭐라고 말했지?" 등 청각 정보 중심의 질의에 대답하지 못함.
    2. Q&A 윈도우 크기가 협소하여 이전 맥락을 포함한 복합 질문에 한계가 있음.
    3. 화면 해설 TTS 도중 사용자가 Q&A 모달을 열고 닫을 때, 영상 오디오 더킹(볼륨이 줄어든 상태)이 원상 복구되지 않고 고정되는 현상이 잔존함.
    4. 모달이 닫힐 때 제공되는 스크린 리더 안내 문구가 불필요하게 길어 노이즈가 됨.
- **해결:**
    1. **대사 인식 컨텍스트 결합 및 상태 기계 파서 도입**: Q&A 시점에 유튜브 원본 자막(VTT) 파일을 전처리하여 `T ± 45초` 영역의 대사 텍스트를 추출해 `dialogueContext`로 믹인. Windows/Unix 개행 문자 및 다중 행 자막을 온전히 대응할 수 있도록 VTT 상태 기계(State Machine) 파서 도입.
    2. **자막 사전 캐싱(Pre-caching)**: 비디오 최초 처리(일반/배치) 완료 시점에 VTT 자막 원본 파일을 캐시 디렉토리에 선제 복사하여, Q&A 수행 시 실시간 자막 다운로드 오버헤드를 100% 제거(캐시 히트).
    3. **컨텍스트 시간 윈도우 동기화**: 화면 해설과 대사 컨텍스트의 탐색 범위를 `T - 120초` ~ `T + 45초` 비대칭 범위로 확장하고, 60초 샘플링 간격의 글로벌 아웃라인을 프롬프트에 조립하여 영상의 시/청각 정보 일치화.
    4. **오디오 더킹 오작동 우회**: 모달 오픈 시 활성 상태의 TTS를 정지하고 볼륨을 복구하며, 모달이 닫힐 때는 TTS 유무에 관계없이 볼륨을 `100`으로 무조건 강제 리셋하여 오디오 잔류 버그 해결.
    5. **안내 문구 단순화**: 모달 종료 피드백을 `질의응답 닫힘`으로 축소하여 접근성 편의성 최적화.
