...
## 6. 오디오 재생 방식 개선 탐색 기록 (2025-11-19)

TTS 음성 해설 재생 시, 기존의 '영상 일시정지' 방식이 사용자의 몰입감을 저해하는 문제를 해결하기 위해 여러 대안을 탐색하고 테스트했습니다.

### 6.1. 목표
- 영상의 흐름을 끊지 않으면서, TTS 음성 해설을 명확하게 전달하는 최적의 사용자 경험 찾기.

### 6.2. 시도한 방식 및 결과

#### 1차 시도: 고급 오디오 더킹 (Advanced Audio Ducking)
- **방법**: TTS 재생 시 영상 볼륨을 15%까지 부드럽게 줄이고, 종료 후 0.5초 뒤에 다시 원래 볼륨으로 복원.
- **결과**:
    - **데스크탑**: 성공적으로 동작하여 매우 부드럽고 전문적인 경험을 제공함.
    - **모바일**: iOS를 포함한 대부분의 모바일 브라우저에서, 보안 정책상 스크립트를 통한 `setVolume` API 호출을 무시함. **작동 실패.**

#### 2차 시도: 모바일 전용 재생 속도 조절 (Playback Rate Control)
- **방법**: 모바일 환경에서만, TTS 재생 시 영상의 속도를 0.75배속으로 느리게 조절.
- **결과**:
    - 재생 속도 변경 시 영상이 순간적으로 끊기는(stuttering) 현상 발생.
    - 영상 소리의 톤이 낮아졌으나, 해설과 명확히 구분될 정도는 아니어서 사용자 경험 개선 효과가 미미함.
    - **폐기 결정.**

#### 3차 시도: 지능형 일시정지/재생 (Intelligent Pause/Resume)
- **방법**: TTS 종료 후, 즉시 재생하지 않고 0.4초의 딜레이를 준 뒤, 영상 시간을 0.5초 뒤로 되감고 재생.
- **결과**: 로직은 정상 동작했으나, 의도적으로 되감는 경험이 사용자에게 직관적으로 와닿지 않고 오히려 혼란을 줄 수 있다는 피드백에 따라 **폐기 결정.**

### 6.3. 추가 조사 및 최종 결론

- **웹 검색**: 'Web Audio API'를 통해 교차 출처(cross-origin) iframe(유튜브)의 오디오를 직접 제어하는 방법을 심층적으로 조사했으나, 브라우저의 **동일 출처 정책(Same-Origin Policy)** 보안 장치로 인해 기술적으로 불가능함을 재확인했습니다.
- **최종 결정**:
    - 여러 실험 결과, 모든 플랫폼에서 가장 안정적이고 예측 가능하게 동작하는 방식은 **'단순 일시정지/재생'** 임을 확인했습니다.
    - 다른 모든 실험적 코드를 제거하고, 코드 구조만 개선된(refactored) '단순 일시정지/재생' 로직으로 최종 확정했습니다. 이는 예측 불가능한 오류를 최소화하고 모든 사용자에게 일관된 경험을 제공하기 위한 최선의 선택입니다.
