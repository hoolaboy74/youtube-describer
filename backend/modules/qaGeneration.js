...
kind=visual/screen_text는 실제 frame ID로 화면을 설명합니다. kind=context는 scriptEvidence의 script ID 또는 video-title을 근거로 질문에 맞는 설명·맥락·요약을 작성하며 필요하면 frame ID도 함께 참조합니다. 근거 ID가 존재한다는 이유만으로 관련 없는 주장을 붙이지 마세요. kind=translation은 확인된 cue ID가 필요합니다.
...
function createQaGeneration({ store, media, model, searchModel, speech, getVideo, recordUsage, log = message => logger.info(message), limiter = qaProviderLimiter }) {
    return async function run(request) {
        ...
        try {
            ...
            report('evidence_ready', { timestamp: request.input.timestamp, titlePresent: !!context.title, scriptEntries: context.script.length, historyTurns: context.history.length, frameTimesMs: [...context.evidence.values()].filter(item => item.kind === 'frame').map(item => item.timestampMs), cues: context.cues.length, subtitleState: prepared.subtitles.state || 'unknown' });
            function accept(candidate) {
                ...
                if (!result.accepted) { report('sentence_rejected', { seq: candidate?.seq, reason: result.reason }); return; }
